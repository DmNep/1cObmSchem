...
Функция СоздатьСхемуPlantUML()
	...
	Схема = Схема + "'Создано обработкой 1c_ObmShem, " + ТекущаяДатаСеанса() + Символы.ПС;
	Схема = Схема + "'Created by processing 1c_ObmShem, " + ТекущаяДатаСеанса() + Символы.ПС;
	Схема = Схема + "'v.0.01  27-04-2024" + Символы.ПС;
	Схема = Схема + "'E-mail: dm-nep-supp@mail.ru" + Символы.ПС;
	Схема = Схема + "'GitHub: github.com/DmNep/1cObmShem" + Символы.ПС;
	...
	Для Каждого СтрокаОбмены Из Объект.Обмены Цикл
		...
		Схема=Схема+?(ЗначениеЗаполнено(БазаИсточник[0].Обозначение),БазаИсточник[0].Обозначение,"")+?(СтрокаОбмены.Двусторонний,"<","")+?(СтрокаОбмены.ТипЛинии="Пунктирная","-","")+"-"+?(ЗначениеЗаполнено(СтрокаОбмены.ЦветЛинии),"[#"+СтрокаОбмены.ЦветЛинии+"]","")+">"+?(ЗначениеЗаполнено(БазаПриемник[0].Обозначение),БазаПриемник[0].Обозначение,"")+ " : "+ СтрокаОбмены.Наименование+Символы.ПС; 
	...
	Схема = Схема + "footer 1c_ObmShem" + Символы.ПС;	
	// Завершаем схему
	Схема = Схема + "@enduml"; 
...
Процедура ЗаписьВ_XML(ПутьКФайлу)
	...
	ТекстКомментария = "";
	ТекстКомментария = ТекстКомментария + "'Создано обработкой 1c_ObmShem, " + ТекущаяДатаСеанса + Символы.ПС;
	ТекстКомментария = ТекстКомментария + "'Created by processing 1c_ObmShem, " + ТекущаяДатаСеанса + Символы.ПС;
	...
	ТекстКомментария = ТекстКомментария + "'GitHub: github.com/DmNep/1cObmShem" + Символы.ПС;
